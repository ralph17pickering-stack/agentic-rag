...
This is the first module of the Agentic RAG Masterclass. No application code exists yet. The user has a local OpenAI-compatible LLM at `http://127.0.0.1:8081/v1` (Chat Completions only), so we skip the OpenAI Responses API entirely and manage threads/history ourselves from the start. Supabase project needs to be created manually by the user.
...
- Create `backend/` with structure: `app/{main,config,dependencies}.py`, `app/routers/`, `app/models/`, `app/services/`
...
- Populates `backend/.env` and `frontend/.env`
...
- `backend/app/dependencies.py` — `get_current_user` dependency using `python-jose` to decode Supabase JWT (HS256, audience "authenticated")
...
- `backend/app/services/supabase.py` — `get_supabase_client(access_token)` that sets user JWT for RLS enforcement

### 6.2 Thread CRUD endpoints
- `backend/app/routers/threads.py` — GET/POST/PATCH/DELETE `/api/threads`
- `backend/app/models/threads.py` — Pydantic request/response models
- **Verify:** Curl CRUD operations with valid JWT

### 6.3 Messages endpoint
- `GET /api/threads/{thread_id}/messages` in `backend/app/routers/chat.py`
- `backend/app/models/messages.py` — Pydantic models
...
### 6.5 LLM service
- `backend/app/services/llm.py` — AsyncOpenAI client pointed at `http://127.0.0.1:8081/v1`, `stream_chat_completion()` async generator
