...
🔴 **Complex** — Touches infrastructure (Caddy), Docker setup, DB schema, backend API, and a new settings UI.
...
```
Internet
  │  HTTPS (Let's Encrypt, or self-signed on localhost)
  ▼
Caddy (port 443/80)
  ├── /* → serves React build (static files from multi-stage build)
  └── /api/* → reverse-proxy → http://backend:8001 (plain HTTP, Docker network)

Backend (FastAPI / uvicorn)
  └── HTTP on port 8001 (internal only, not exposed to host)

Frontend (built React)
  └── Same-origin requests to /api/* (no CORS needed in production)
```

**Key decisions:**
- Backend runs **plain HTTP only** — no self-signed certs, no uvicorn SSL. Caddy terminates TLS for everything.
...
- Settings writes are restricted to service_role (admin) — the backend uses the service role client for mutations.
...
# Output stage — just the dist/ folder, no runtime
FROM scratch AS dist
COPY --from=build /app/dist /dist
```

The final `dist/` is copied into a named volume that Caddy serves. The `scratch` stage means the image has zero runtime footprint — it's only used to produce the build artifact.
...
services:
  backend:
    ...
    networks:
      - app
    # Not exposed to host — only reachable via Caddy
    expose:
      - "8001"
  ...
  frontend:
    ...
    # Build-only service — no runtime container
    profiles: ["build"]
...
### Task C1: Database migration 016 — settings table
...
- Removed `backend_internal_hostname` — not needed since backend is always plain HTTP behind Caddy.
...
Notes:
- `cert_*` fields are read-only from the API perspective — they're populated by the cert-info endpoint.
...
async def get_cert_info(user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(502, f"Could not read cert from {hostname}:443 — {e}")
```

Key differences from original:
- **No `regenerate-cert` endpoint** — Caddy manages certs automatically. There's nothing to regenerate manually.
...
Layout — single scrollable page with two cards:
...
No "Regenerate Certificate" button — Caddy handles renewal automatically. The UI is informational + hostname config only.
...
**File**: `frontend/.env.EXAMPLE` — no changes needed (VITE_API_URL is already there; it stays empty when behind Caddy).
...
1. **Sub-Plan E** (Config) — add env vars to config.py and .env.EXAMPLE first, since other tasks depend on them
2. **Sub-Plan B** (CORS) — make CORS configurable before Docker changes
3. **Sub-Plan A** (Infrastructure) — Dockerfiles, docker-compose, Caddyfile, build script
4. **Sub-Plan C** (Settings DB + API) — migration, models, router
5. **Sub-Plan D** (Frontend UI) — settings page, hook, navigation
...
- [ ] CORS still works for local dev (`npm run dev` on :5173 → backend on :8001)
...
- `caddy:2-alpine` needs persistent data volume for cert storage — losing it means re-provisioning certs on restart.
...
- Frontend build args (`ARG` in Dockerfile) are baked in at build time — changing Supabase URL requires rebuilding the frontend image.
