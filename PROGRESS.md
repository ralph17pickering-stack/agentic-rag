...
- [ ] Projects: Both chat and documents can live in projects, and projects can be shared between users. Projects can be heirarchical, with sub-projects inheriting the parent project's documents.
- [ ] Before chunking, imported data should be cleaned and restructured to remove any unnecessary whitespace and formatting that might interfere with the embedding process, while reformatting meaningful formatting e.g. tables so that they provide valuale context after embedding.
- [ ] Chunking should respect the document's natural boundaries rather than blindly splitting on word count or token count. Tables should stay whole, headings must remain with the content.
- [ ] Metadata creation: For each chunk, there should be a summary generated by the LLM, keywords extracted and stored in a keywords table, and generate hypothetical questions generated that the chunk might answer.
- [ ] Reasoning engine: User queries should not simply generate vector database queries that then are blindly added to the context. Instead, they should be processed through a planner that figures out what the user really needs, then chooses the appropriate tools to use. A conditional router will route to the appropriate tool to a multi-agent system that can reason about the query and provide the best answer from different sources and skills.